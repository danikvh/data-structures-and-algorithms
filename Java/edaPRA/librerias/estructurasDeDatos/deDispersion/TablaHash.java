...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        /* COMPLETAR */
        ListaConPI<EntradaHash<C, V>>[] aux = elArray; //referencia a elArray
        elArray = new LEGListaConPI[siguientePrimo(elArray.length * 2)];
        for (int i = 0; i < elArray.length; i++) { //inicializar array con cubetas
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        
        for (int i = 0; i < aux.length; i++) { //introducir los elementos en el nuevo array
            for (aux[i].inicio(); !aux[i].esFin(); aux[i].siguiente()) {
                EntradaHash<C, V> elemento = aux[i].recuperar();
                elArray[indiceHash(elemento.clave)].insertar(
                    new EntradaHash<C, V>(elemento.clave, elemento.valor));
            }
        }
        //Solución Gideon
        // for(ListaConPI<EntradaHash<C, V>> lista: aux) {
            // for (lista.inicio(); !lista.esFin(); lista.siguiente()) {
                ...
                // elArray[indiceHash(elemento.clave)].insertar(
                    // new EntradaHash<>(elemento.clave, elemento.valor));
            // }
        // }
    }
    ...
    public final double desviacionTipica() {
        /* COMPLETAR */
        double media = this.factorCarga();
        double res = 0.0;
        for (int i = 0; i < elArray.length; i++) {
            res = res + Math.pow(elArray[i].talla() - media, 2);
        }      
    ...
    public final double costeMLocalizar() {
        ...
        double colisiones = 0;
        for (int i = 0; i < elArray.length; i++) {
            colisiones += (elArray[i].talla() * (elArray[i].talla() - 1)) / 2;
        }
        return colisiones / this.talla; 
    }
    ...
    //Examen
    public ListaConPI<C> clavesColisionadas2() {
        ListaConPI<C> res = new LEGListaConPI<C>();
        ListaConPI<C> claves = this.claves();
        Map<C,Integer> aux = new TablaHash<C, Integer>(this.talla);
        for (claves.inicio(); !claves.esFin(); claves.siguiente()) {
            C c = claves.recuperar();
            Integer contador = aux.recuperar(c);
            if (contador == null) { 
                aux.insertar(c, 1); 
            }
            else {
    ...
    public ListaConPI<C> clavesColisionadas() {
        ...
        //claves.inicio();
        for (int i = 0; i < elArray.length; i++) {
            if (elArray[i].talla() > 1) {
                ListaConPI<EntradaHash<C,V>> aux = elArray[i];
                aux.inicio(); aux.siguiente();
                while (!aux.esFin()) {
                    res.insertar(aux.recuperar().clave);
                    aux.siguiente();
                }
            }
            //claves.siguiente();
