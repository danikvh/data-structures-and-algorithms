...
public abstract class Grafo {
    ...
    public Arista[] kruskal() {       
        ...
        Arista[] res = new Arista[numVertices() - 1]; //E'
        int cardinal = 0; //cardinal E'
        //int[] array = this.toArrayBFS();
        ColaPrioridad<Arista> aristasFactibles = new MonticuloBinarioR0<Arista>(); //aristasFactibles
        ...
        while (cardinal < numVertices() - 1 && !aristasFactibles.esVacia()) {
            ...
            ccV = cc.find(arista.getOrigen());
            ccW = cc.find(arista.getDestino());
            if (ccV != ccW) {
                res[cardinal++] = arista;
                cc.union(ccV, ccW);
            }
        ...
        if (cardinal == numVertices() - 1) { return res; }
        return null;
    }
    ...
    public int[] gradosMST() {
        ...
        for (int i = 0; i < aux.length; i++) {
            int origen = aux[i].getOrigen();
            int destino = aux[i].getDestino();
            res[origen]++;
            res[destino]++;
        }
        return res;
