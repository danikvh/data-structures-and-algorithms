...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        ...
        ListaConPI<Termino> c = index.claves(); //lista de los términos
        
        ListaConPI<BuscadorDeLaBibl.Posting> valor;
        for(c.inicio(); !c.esFin(); c.siguiente()) {
            Termino clave = c.recuperar(); //recupera un termino de la lista
            valor = index.recuperar(clave); //devuelve el valor (cubeta) asociado a la clave
            if (valor.talla() == 1) {
                res.insertar(clave);
            }
        }
